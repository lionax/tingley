<?php
	 ...
	 * 
	 * Copyright (C) 2009-2010 Nikolaus Themessl
	 *
	 *
	...
	**/
	...
	$version['string'] = $version['major'] . "." . $version['minor'] . "." . $version['bug'] . " Beta";
	...
	if (trim($_GET['mod']) == '') {
		$load_module = $db->selectOne('menu', 'mod', "`home`=1 AND `language`='".$current_language."'");
		if ($load_module == '')
			$load_module = $db->selectOne('menu', 'mod', "`home`=1 AND `language`=''");
	...
	$smarty->assign('menu', $menu->getMenu(0, null, null, $include));
