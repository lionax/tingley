...
class poll
{
	...
	function vote($identification, $pollID, $QIDs)
	{
		...
		foreach($QIDs as $QID)
		{
			$sql=$this->db->query("UPDATE ".MYSQL_TABLE_PREFIX."poll_questions AS Q SET Q.count=Q.count+1 WHERE Q.pollID='".(int)$pollID."' AND Q.ID='".$QID."';");
		}
		return $sql;
	}
	...
	{
		$sql="INSERT INTO ".MYSQL_TABLE_PREFIX."poll (name, date, button) VALUES ( '".secureMySQL($text)."','".time()."', '".$buttontype."');";
	...
	{
		$sql="UPDATE ".MYSQL_TABLE_PREFIX."poll SET name='".secureMySQL($text)."' , button='".$button."' WHERE ID='".(int)$pollID."';";
		return $this->db->query($sql);
	}

	function updateQuestion($text, $QID)
	{
		$sql="UPDATE ".MYSQL_TABLE_PREFIX."poll_questions SET text='".secureMySQL($text)."' WHERE ID='".$QID."';";
		return $this->db->query($sql);
	}

	function deletePoll($PID)
	{
		$sql="DELETE FROM ".MYSQL_TABLE_PREFIX."poll WHERE ID='".$PID."';";
		return $this->db->query($sql);
	}

	function deleteQuestionByPID($PID)
	{
		$sql="DELETE FROM ".MYSQL_TABLE_PREFIX."poll_questions WHERE pollID='".$PID."';";
		return $this->db->query($sql);
	}
	
	function deleteQuestionByID($ID)
	{
		$sql="DELETE FROM ".MYSQL_TABLE_PREFIX."poll_questions WHERE ID='".$ID."';";
		return $this->db->query($sql);
	}
	...
	function switchActive($ID)
	{
		...
		$active=($active==1)?0:1;
		$sql="UPDATE ".MYSQL_TABLE_PREFIX."poll SET active='".$active."' WHERE ID='".$ID."';";
		return $this->db->query($sql);
	}
