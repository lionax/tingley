...
<script type="text/javascript">
	...
	function showInfo(day) {
		...
		info.load('ajax_request.php?mod=media&file=top-dls-per-day.ajax&start_day='+day+'&end_day='+day+'&month={$dl_stat.month_value}');
...
{if count($dl_stat.days) > 0}
	...
	{if !$smarty.get.downloadid > 0}
		...
		<script type="text/javascript">
			$("#divTopThisMonth").load('ajax_request.php?mod=media&file=top-dls-per-day.ajax&start_day=1&end_day={$dl_stat.month_last_day}&month={$dl_stat.month_value}&nolimit');
		</script>
	{/if}
{/if}
