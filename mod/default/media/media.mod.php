	...
	{	
		case 'statistic':
			if ($rights->isAllowed($mod, 'manage')) {
				...
				$selected_month_ts = mktime(0, 0, 0, $selected_month, 15, date("Y"));
				...
				$stat['month'] = date("M", mktime(0, 0, 0, $month, 1, date("Y")));
				$stat['month_value'] = $month;
				$stat['month_last_day'] = date("t", mktime(0, 0, 0, $month, 1, date("Y")));
				$max = 0;
				...
				$stat['export_url'] = 'ajax_request.php?mod=media&amp;file=stat.export&amp;month='.$month;
					...
					if (isVisible($dl['categoryid'])) {
						if (!isset($_GET['statistic'])) {
							...
							if (!file_exists($path)) {
								...
								$dl['url'] = "javascript:window.open('".$path."', '_blank', 'width=350, height=200, resizable=yes');";
							...
							if ($rights->isAllowed($mod, 'manage')) {
								...
								$stat['export_url'] = 'ajax_request.php?mod=media&amp;file=stat.export&amp;downloadid='.$downloadid.'&amp;month='.$month;	
								$smarty->assign('dl_stat', $stat);
							}
						}
