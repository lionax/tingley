<form action="" method="post">
	{foreach from=$form.elements item=element}
		...
		{if $element.type == 4}
			<table width="100%" border="0">
				<tr>
					<td width="150">{if $element.required == 1}*{/if}{$element.title}:</td>
					<td><input type="checkbox" name="field_{$element.elementid}" value="1"{if $submit[$element.elementid] == 1} checked="checked"{/if} /></td>
				</tr>
			</table>
		{/if}
		{if $element.type == 5}
	...
	<p>
		<input type="submit" name="submit_form" value="{if $form.submit != ''}{$form.submit}{else}{$lang.submit}{/if}"{if $submitted} disabled="disabled"{/if} />
	</p>
</form>