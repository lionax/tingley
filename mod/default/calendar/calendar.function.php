<?php
	...
	function makeEntryArrayQuarters($entries, &$maxCols) {
		...
		if (count($entries) > 0) {
			...
			foreach ($entries as $entry) {
				...
				global $db;
				$color = $db->selectOneRow('hfh_calendar_categories', '*', "`categoryId`='".$entry['categoryId']."'");
	...
	function makeEntryArrayHours($entries, &$maxCols) {
		...
		if (count($entries) > 0) {
			foreach ($entries as $entry) {
				...
				$rowspan = ceil(($entry['end']-$entry['start'])/60/60);
	...
	function getTodaysBirthdays($today) {
		...
		$birthdays = $db->selectList(MYSQL_TABLE_PREFIX."users", "*", "FROM_UNIXTIME(`birthday`, '%d.%m') = " . date("d.m", $today));
	...
	function getCategories() {
		...
		return $db->selectList('hfh_calendar_categories', '*');
	}
?>